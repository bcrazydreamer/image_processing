...
Pixel simulator
'''
def simulate_pixel(pixel_array,range_width):
    simulated_output = ""
    for row in pixel_array:
        row_simulated_output = ""
        for pixel_set in row:
            row_simulated_output += color(pixel_symbol,pixel_set,range_width)
        row_simulated_output += "\n"
...
Image simulator
'''
def simulate_image(image,range_width):
    pixel_array = genrate_pixel_array(image,range_width)
    simulated_image = simulate_pixel(pixel_array,range_width)
...
if __name__=='__main__':
    ...
    try:
        while True:
            ...
            image = process_image(image,150,"color")
            print("\033c"+image)
    except:
        print("\033c")
        print("Video closed")
